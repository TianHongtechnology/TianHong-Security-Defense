...
#include "utils.hpp"
#include "../Log.h"
#define DbgPrintEx(a,b,...) DbgPrintWithTag(__VA_ARGS__)
#define DbgPrint(...) DbgPrintWithTag(__VA_ARGS__)
...
namespace k_hook
{
	...
	NTSTATUS modify_trace_settings(trace_type type)
	{
		...
		// 申请保存名称的空间
		wchar_t* provider_name = (wchar_t*)ExAllocatePool2(NonPagedPool, 256 * sizeof(wchar_t), tag);
		if (!provider_name)
		{
